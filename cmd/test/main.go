...
func run(cmd *cobra.Command, args []string) {
	...
	case "swift":
		...
		authTenantID := os.Getenv("OS_PROJECT_NAME")
		if authTenantID == "" {
			logg.Fatal("OS_PROJECT_NAME environment variable is required when using Swift storage driver")
		}
		...
		authConfig = `{"type":"keystone","params":{"oslo_policy_path":"policy.json"}}`
...
func executeWriteTrivyReport(ctx context.Context, sd keppel.StorageDriver, account models.ReducedAccount, args []string) {
	if len(args) != 3 {
		logg.Fatal("write-trivy-report requires: <repo-name> <digest> <payload-json>")
	}
