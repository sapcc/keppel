	...
	assert.HTTPRequest{
		...
		Body: assert.JSONObject{
			"account": assert.JSONObject{
				...
				"rbac_policies":  []assert.JSONObject{},
				"validation": assert.JSONObject{
					"rule_for_manifest": "has(labels.foo) ? 1 : 0",
				},
			},
		},
		ExpectStatus: http.StatusUnprocessableEntity,
	...
	assert.HTTPRequest{
		...
		Body: assert.JSONObject{
			"account": assert.JSONObject{
				...
				"rbac_policies":  []assert.JSONObject{},
				"validation": assert.JSONObject{
					"rule_for_manifest": "has(labels.foo) && has(labels.bar)",
					"required_labels":   []string{"qux", "quux"},
				},
			},
		},
		ExpectStatus: http.StatusUnprocessableEntity,
		ExpectBody:   assert.StringData("required labels [\"qux\" \"quux\"] do not match rule for manifest \"has(labels.foo) && has(labels.bar)\"\n"),
	}.Check(t, h)
	...
	assert.HTTPRequest{
		...
		ExpectBody: assert.JSONObject{
			"account": assert.JSONObject{
				...
				"rbac_policies":  []assert.JSONObject{},
				"validation": assert.JSONObject{
					"rule_for_manifest": "has(labels.baz) && has(labels.qux)",
					"required_labels":   []string{"baz", "qux"},
				},
			},
		},
	...
	assert.HTTPRequest{
		...
		Body: assert.JSONObject{
			"account": assert.JSONObject{
				...
				"rbac_policies":  []assert.JSONObject{},
				"validation": assert.JSONObject{
					"rule_for_manifest": "has(labels.quux)",
