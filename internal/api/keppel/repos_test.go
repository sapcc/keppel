...
func TestReposAPI(t *testing.T) {
	...
	for idx := 1; idx <= 10; idx++ {
		...
		must.SucceedT(t, keppel.MountBlobIntoRepo(ctx, s.DB, blob, filledRepo))
	}
	...
	deletingTagPolicyJSON := `{"match_repository":".*","match_tag":"tag2","block_delete":true}`
	...
	s.RespondTo(ctx, "DELETE /keppel/v1/accounts/test1/repositories/repo1-3", withPerms("delete:tenant1,view:tenant1")).
		ExpectText(t, http.StatusConflict, fmt.Sprintf("cannot delete manifest %s because it is protected by tag policy ({\"match_repository\":\".*\",\"match_tag\":\"tag2\",\"block_delete\":true})\n", test.DeterministicDummyDigest(2)))
