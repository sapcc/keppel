...
func (s SecurityScanPolicySet) EnrichReport(payload *trivy.ReportPayload) (models.VulnerabilityStatus, error) {
	...
	noStatus := models.ErrorVulnerabilityStatus

	if payload.Format != "json" {
		return noStatus, fmt.Errorf("cannot run EnrichReport() on a ReportPayload with .Format = %q", payload.Format)
	}

	// decode relevant fields from report
	parsedReport, err := trivy.UnmarshalReportFromJSON(payload.Contents)
	if err != nil {
		return noStatus, fmt.Errorf("cannot parse Trivy vulnerability report: %w", err)
	}
	...
	for _, result := range parsedReport.Results {
		for _, vuln := range result.Vulnerabilities {
			...
			if !ok {
				return noStatus, fmt.Errorf("vulnerability severity with name %q returned by Trivy is unknown and cannot be mapped", vuln.Severity)
			}
	...
	if len(applicablePolicies) > 0 {
		...
		if err != nil {
			return noStatus, fmt.Errorf("cannot serialize enriched Trivy vulnerability report: %w", err)
		}
	}
