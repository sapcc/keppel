...
func RenderValidationPolicy(account models.ReducedAccount) *ValidationPolicy {
	...
	re := regexp.MustCompile(`^has\(labels\.([a-zA-Z_][a-zA-Z0-9_]*)\)(\s*&&\s*has\(labels\.([a-zA-Z_][a-zA-Z0-9_]*)\))*$`)
	if re.MatchString(account.RuleForManifest) {
		policy.RequiredLabels = generateRequiredLabels(account.RuleForManifest)
	}
...
func (v ValidationPolicy) ApplyToAccount(account *models.Account) *RegistryV2Error {
	...
	if v.RuleForManifest != "" && v.RequiredLabels != nil {
		generatedRule := generateRuleForManifest(v.RequiredLabels)
	...
	if v.RuleForManifest != "" {
		...
		account.RuleForManifest = generateRuleForManifest(v.RequiredLabels)
	}

	return nil
}

func generateRequiredLabels(ruleForManifest string) []string {
	re := regexp.MustCompile(`has\(labels\.([a-zA-Z_][a-zA-Z0-9_]*)\)`)
	matches := re.FindAllStringSubmatch(ruleForManifest, -1)
...
func generateRuleForManifest(requiredLabels []string) string {
	var propositions []string
	for _, label := range requiredLabels {
		propositions = append(propositions, fmt.Sprintf("has(labels.%s)", label))
	}
...
func BuildManifestValidationAST(celExpression string) (*cel.Env, *cel.Ast, error) {
	env, err := cel.NewEnv(cel.Variable("labels", cel.MapType(cel.StringType, cel.StringType)))
	if err != nil {
		return nil, nil, err
	}
