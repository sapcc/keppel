...
type RegistryV2Error struct {
	...
	Message marshalableError    `json:"message"`
...
// Unwrap implements the interface implied by errors.As().
func (e *RegistryV2Error) Unwrap() error {
	return e.Message.inner
}

// A container for an error that can be marshaled as a JSON string while retaining the full error value at runtime.
