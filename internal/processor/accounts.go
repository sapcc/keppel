...
func (p *Processor) CreateOrUpdateAccount(ctx context.Context, fd keppel.FederationDriver, userInfo audittools.UserInfo, r *http.Request, subleaseHeader string, account keppel.Account) (models.Account, *keppel.RegistryV2Error) {
	...
	if originalAccount != nil {
		if account.PlatformFilter != nil && !originalAccount.PlatformFilter.IsEqualTo(account.PlatformFilter) {
		...
		case keppel.OnFirstUseStrategy:
			...
			upstreamPlatformFilter, err := p.GetPlatformFilterFromPrimaryAccount(ctx, targetAccount)
	...
	if originalAccount == nil {
		...
		if targetAccount.UpstreamPeerHostName != "" {
			subleaseToken, err := keppel.SubleaseTokenFromRequest(subleaseHeader)
			if err != nil {
