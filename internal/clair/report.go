...
func (c *Client) GetVulnerabilityReport(ctx context.Context, manifestDigest digest.Digest) (*VulnerabilityReport, error) {
	//if we faked the indexing report, we also need to fake the vulnerability
	//report (see comment in c.submitManifest())
	if c.isEmptyManifest[manifestDigest] {
	...
	reqURL := c.requestURL("matcher", "api", "v1", "vulnerability_report", manifestDigest.String())
