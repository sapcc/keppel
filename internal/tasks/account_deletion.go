...
func (j *Janitor) deleteMarkedAccount(ctx context.Context, accountName models.AccountName, labels prometheus.Labels) error {
	...
	err = j.DeleteAccount(ctx, *accountModel, actx)
	if err != nil {
		return fmt.Errorf("could not delete account %q: %w", accountName, err)
	}
	return nil
}
...
func (j *Janitor) DeleteAccount(ctx context.Context, account models.Account, actx keppel.AuditContext) error {
	...
	if len(nextManifests) > 0 {
		...
		logg.Info("cleaning up account %q: need to delete %d manifests in this cycle", account.Name, len(nextManifests))
		for _, rm := range nextManifests {
			...
			repo, err := keppel.FindRepository(j.db, rm.RepositoryName, account.Name)
			if err != nil {
				return fmt.Errorf("while deleting manifest %q in repository %q: could not find repository in DB: %w",
					rm.Digest, rm.RepositoryName, err)
			}
			err = j.processor().DeleteManifest(ctx, account.Reduced(), *repo, parsedDigest, actx)
			if err != nil {
	...
	err = j.sd.CleanupAccount(ctx, account.Reduced())
	if err != nil {
	...
	if userInfo := actx.UserIdentity.UserInfo(); userInfo != nil {
		j.auditor.Record(audittools.EventParameters{
			...
			Target:     processor.AuditAccount{Account: account},
		})
	}
