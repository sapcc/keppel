...
type manifestDataSlice []*manifestData
...
func (j *Janitor) executeGCPolicies(account keppel.Account, repo keppel.Repository, policies []keppel.GCPolicy) error {
	...
	var manifests manifestDataSlice
	...
	err = sqlext.WithPreparedStatement(j.db, query, manifests.persistGCStatus)
	if err != nil {
...
func (manifests manifestDataSlice) persistGCStatus(stmt *sql.Stmt) error {
	for _, m := range manifests {
		if m.IsDeleted {
			continue
		}
		...
		_, err = stmt.Exec(string(gcStatusJSON), m.repoID, m.Manifest.Digest)
		if err != nil {
			return err
		}
	}
