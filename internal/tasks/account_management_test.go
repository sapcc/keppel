...
func TestAccountManagementBasic(t *testing.T) {
	...
	expectSuccess(t, managedAccountsJob.ProcessOne(s.Ctx))
	tr.DBChanges().AssertEqualf(`
      UPDATE accounts SET is_deleting = TRUE, next_deletion_attempt_at = %d WHERE name = 'abcde';
    `,
		s.Clock.Now().Unix(),
	)
...
func TestAccountManagementWithComplexDeletion(t *testing.T) {
	...
	tr.DBChanges().AssertEqualf(`
      UPDATE accounts SET next_blob_sweep_at = %[1]d WHERE name = 'abcde';
      UPDATE blobs SET can_be_deleted_at = %[1]d WHERE id = 1 AND account_name = 'abcde' AND digest = '%[2]s';
      UPDATE blobs SET can_be_deleted_at = %[1]d WHERE id = 2 AND account_name = 'abcde' AND digest = '%[3]s';
      UPDATE blobs SET can_be_deleted_at = %[1]d WHERE id = 3 AND account_name = 'abcde' AND digest = '%[4]s';
    `,
		s.Clock.Now().Unix(),
		image.Layers[0].Digest.String(),
		image.Layers[1].Digest.String(),
