...
func TestAccountManagementDriver(t *testing.T) {
	...
	createManagedAccountsJob := j.EnforceManagedAccounts(s.Registry)

	// we haven't configured any account, so it should do nothing
	s.AMD.ConfigPath = "./fixtures/account_management_empty.yaml"
	expectError(t, sql.ErrNoRows.Error(), createManagedAccountsJob.ProcessOne(s.Ctx))
	tr.DBChanges().AssertEmpty()

	// configure an account to create
	s.AMD.ConfigPath = "./../drivers/basic/fixtures/account_management.yaml"
	s.Clock.StepBy(1 * time.Hour)

	// we only have one account defined which should be created
	expectSuccess(t, createManagedAccountsJob.ProcessOne(s.Ctx))
	...
	s.AMD.ConfigPath = "./fixtures/account_management_error.yaml"
	s.Clock.StepBy(2 * time.Hour)
	expectError(t, processor.ErrAccountNameEmpty.Error(), createManagedAccountsJob.ProcessOne(s.Ctx))
	tr.DBChanges().AssertEmpty()
