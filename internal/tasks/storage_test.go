...
func TestSweepStorageBlobs(t *testing.T) {
	...
	mustDo(t, s.SD.AppendToBlob(s.Ctx, account, storageID3, 1, &sizeBytes, bytes.NewReader(testBlob3.Contents)))
	...
	mustDo(t, s.SD.AppendToBlob(s.Ctx, account, storageID4, 1, &sizeBytes, bytes.NewReader(testBlob4.Contents)))
