...
| Metric | Explanation |
| ------ | ----------- |
| `keppel_successful_blob_sweeps`<br>`keppel_failed_blob_sweeps`<br>`keppel_storage_sweeps` | Counters for account-level operations. One increment equals one account. |
| `keppel_blob_mount_sweeps`<br>`keppel_successful_manifest_syncs`<br>`keppel_failed_manifest_syncs` | Counters for repository-level operations. One increment equals one repository. |
