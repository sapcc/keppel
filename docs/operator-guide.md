...
| Account platform filter sync | Takes a replica account, asks the peer holding the primary account for its current platform filter and updates the local platform filter if necessary. This keeps replica accounts in sync when the primary account's platform filter is ever updated.<br><br>*Rhythm:* every hour (per replica account)<br>*Clock:* database field `accounts.next_platform_filter_sync_at`<br>*Signal:* Prometheus counter `keppel_account_platform_filter_syncs` |
