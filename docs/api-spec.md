...
| `manifests[].trivy_scan_error` | string | Only shown if `trivy_vulnerability_status` is `Error` or `Unsupported`. Contains the error message from Trivy that explains why this image could not be scanned (for status `Error`) or an error message from Keppel that explains why this image was not submitted to Trivy (for status `Unsupported`). When `vulnerability_status` is `Error` or `Unsupported` because scanning failed for an image referenced in this manifest, the error message will be shown on the referenced manifest instead of on this manifest. |
| `truncated` | boolean | Indicates whether [marker-based pagination](#marker-based-pagination) must be used to retrieve the rest of the result. |
