...
```json
{
  "accounts": [
    ...
    },
    {
      ...
      "validation": {
        "rule_for_manifest": "has(labels.maintainers) && has(labels.source_repo)",
      }
    }
  ]
}
...
- Providing `validation.rule_for_manifest` with a valid [CEL expression](https://cel.dev/) that outputs a boolean. As of now, `labels` is the only variable that can be used for the expression, e.g. `has(labels.foo) || has(labels.bar)`.
