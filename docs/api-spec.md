...
- Providing `validation.rule_for_manifest` with a valid CEL expression that outputs a boolean. As of now, `labels` is the only variable that can be used for the expression, i.e. `has(labels.foo) || has(labels.bar)`.
